...
// import { Link } from "wouter" // not used
const nilsHolgerLogo = "/nils-holger-logo.png"; // ✅ use the one in client/public
...
export default function Header({ className = "" }: HeaderProps) {
  ...
  const scrollToSection = (sectionId: string) => {
    if (
      sectionId.toLowerCase() === "karriär" ||
      sectionId.toLowerCase() === "career"
    ) {
    ...
    if (
      sectionId.toLowerCase() === "referenser" ||
      sectionId.toLowerCase() === "references"
    ) {
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 2xl:h-[160px] xl:h-[140px] lg:h-[120px] md:h-[110px] sm:h-[90px] h-[80px] transition-transform duration-300 ease-in-out ${
      ...
      >
        <div className="h-full w-full relative">
          ...
          >
            ...
            >
              <img
                ...
                onError={() =>
                  console.error("Logo failed to load:", nilsHolgerLogo)
                }
              />
            </button>
          </div>
          ...
          <div className="absolute top-0 h-full w-full flex items-center z-10">
            <div
              className={`flex-1 flex px-4 transition-all duration-300 ${
                isScrolled
            ...
            >
              <nav
                className={`hidden 2xl:flex items-center space-x-6 ${isScrolled ? "bg-[#251104]/60 backdrop-blur-sm rounded-lg px-4 py-2" : ""}`}
              >
                {t.nav.map((item, i) => (
                  <button
                    key={item}
              ...
              <nav
                className={`hidden xl:flex 2xl:hidden items-center space-x-3 ${isScrolled ? "bg-[#251104]/60 backdrop-blur-sm rounded-lg px-4 py-2" : ""}`}
              >
                {t.nav.map((item, i) => (
                  <button
                    key={item}
              ...
              <nav
                className={`hidden lg:flex xl:hidden items-center space-x-2 ${
                  isScrolled
      ...
      {isMobileMenuOpen && (
        ...
        >
          <div
            className="fixed top-0 left-0 right-0 bg-[#251104] shadow-lg pt-20"
